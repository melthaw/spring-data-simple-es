package in.clouthink.daas.es.repository.impl;

import com.fasterxml.jackson.databind.ObjectMapper;
import in.clouthink.daas.es.exception.Catching;
import in.clouthink.daas.es.exception.DataNotFoundException;
import in.clouthink.daas.es.exception.IncorrectResultSizeDataAccessException;
import in.clouthink.daas.es.model.MutableIdentityProvider;
import in.clouthink.daas.es.repository.EsCrudRepository;
import in.clouthink.daas.es.repository.EsTemplate;
import in.clouthink.daas.es.repository.Fields;
import in.clouthink.daas.es.repository.SearchSourceBuilderBuilder;
import in.clouthink.daas.es.repository.metadata.IndexMetadata;
import in.clouthink.daas.es.repository.metadata.IndexMetadataParser;
...
public abstract class SimpleEsCrudRepository<T extends MutableIdentityProvider<String>, R extends PageableSearchRequest>
    ...
    protected Page<T> doSearch(R request, Fields fields) throws Exception {
        ...
        return new PageImpl<>(searchResultList, request.resolvePageable(), searchResponseHits.getTotalHits().value);
    }
    ...
    protected Page<T> doGetAll(PageableSearchRequest request, Fields fields) throws Exception {
        ...
        return new PageImpl<>(searchResultList, pageable, searchResponseHits.getTotalHits().value);
    }
    ...
    protected T doGetFirst(SearchSourceBuilder searchSourceBuilder) throws Exception {
        ...
        //handle result
        SearchHits searchResponseHits = searchResponse.getHits();
        if (searchResponseHits.getTotalHits().value == 0) {
            return null;
        }
    ...
    protected T doGetOne(SearchSourceBuilder searchSourceBuilder) throws Exception {
        ...
        if (searchResponseHits.getTotalHits().value > 1) {
            throw new IncorrectResultSizeDataAccessException(1, searchResponseHits.getTotalHits().value);
        }
